...
df['is_dupe']=df.duplicated()
df['is_dupe']=np.where(df['is_dupe']==True,1,0)
df
...
def load_data(messages_path,categories_path):
    '''
    load the messages and categories from the available csv files
    param: messages_path
...
def save_data(df,database_filename):
    '''
    save the dataframe into sqlite dd
    '''
    engine = create_engine('sqlite:///Disaster_messages.db')
    df.to_sql('Disaster_messages', engine, index=False)
